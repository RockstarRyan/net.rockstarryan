...
// V2: Add localStorage capability (similar to KeyConnect) towards bottom
...
function initializeMasterScoresEmpty() {
	...
	for (var i = 0; i < game_info.length; i++) { // For each game (dimension 1)
		...
		master_scores_empty[i] = game;
	}
	return master_scores_empty;
}
function updateMasterScores(game, no, player, index, newValue, id) { // where index=0 is playerName, others are round scores
	...
	if (index != 0) {			// Change to round score
		if (newValue === '-') {
			...
			localStorage.setItem('master_scores',JSON.stringify(master_scores)); // added in V2
		}
...
function setGameInfo(game,key,newValue) {
	var index = getIndex(key);
	for (var i = 0; i < game_info.length; i++) {
		if (game_info[i][0][0] == game) {
			game_info[i][index[0]][index[1]] = newValue;
			localStorage.setItem('game_info',JSON.stringify(game_info)); // added in V2 2026-05-19
...
function initialize_storage() {
	if (is_storage_available("localStorage")) {
		// gameinfo_key and settings_info are part of app, but outside local storage.

		// For game_info and master_scores, retrieve if set, otherwise keep default (set in prev. lines)
		if (localStorage.getItem("game_info")) {
			console.log("game_info retrieved from localStorage");
			game_info = JSON.parse(localStorage.getItem('game_info'));
		} else {
			console.log("game_info NOT retrieved from localStorage - welcome, new user!");
			localStorage.setItem('game_info',JSON.stringify(game_info));
		}

		if (localStorage.getItem("master_scores")) {
			console.log("master_scores retrieved from localStorage");
...
function clear_storage() {
	if (confirm("Are you sure you want to clear your stored scores and game configurations? This action can NOT be undone.")) {
		...
		//get('main.active .msg_success').innerText = 'Successfully cleared localStorage.';
		//get('main.active .msg_success').className = 'msg_success success active';
